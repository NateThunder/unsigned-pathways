...
const MODEL_PATH = "/3d/Meshy_AI_Vintage_Echo_Verb_Ped_0829040750_optimized.glb";

function EchoPedal({ motionAllowed }: { motionAllowed: boolean }) {
  ...
  useFrame(({ clock, pointer }, delta) => {
    ...
    const targetX = motionAllowed ? 0.42 + pointer.y * 0.08 : 0.42;
    const targetY = motionAllowed
      ? -0.3 + pointer.x * 0.12 + Math.sin(clock.elapsedTime * 0.45) * 0.08
      : -0.3;
    ...
    object.position.y = motionAllowed
      ? Math.sin(clock.elapsedTime * 0.7 + 0.8) * 0.045
      : 0;
  });

  return (
    <group ref={group} rotation={[0.42, -0.3, 0.04]}>
      <primitive object={model.clone} scale={model.scale} />
    </group>
  );
}

export function YouthPedalScene() {
      ...
      {shouldMount && (
        ...
        >
          ...
          <Suspense fallback={null}>
            <EchoPedal motionAllowed={motionAllowed} />
          </Suspense>
        </Canvas>
      )}
    </div>
