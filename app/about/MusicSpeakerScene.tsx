...
const MODEL_PATH = "/3d/Meshy_AI_Speaker_Driver_0829034426_optimized.glb";

function SpeakerDriver({ motionAllowed }: { motionAllowed: boolean }) {
  ...
  const model = useMemo(() => {
    ...
    clone.traverse((object) => {
      ...
      const geometrySize = object.geometry.boundingBox?.getSize(
        new THREE.Vector3(),
      );
      rankedMeshes.push({
        mesh: object,
        span: geometrySize?.length() ?? 0,
      });
    });
  ...
  useFrame(({ clock, pointer }, delta) => {
    ...
    const targetX = motionAllowed ? 0.24 + pointer.y * 0.08 : 0.24;
    const targetY = motionAllowed
      ? -0.36 + pointer.x * 0.12 + Math.sin(clock.elapsedTime * 0.45) * 0.08
      : -0.36;
    ...
    object.position.y = motionAllowed
      ? Math.sin(clock.elapsedTime * 0.7) * 0.045
      : 0;
  });

  return (
    <group ref={group} rotation={[0.24, -0.36, -0.08]}>
      <primitive object={model.clone} scale={model.scale} />
    </group>
  );
}

export function MusicSpeakerScene() {
      ...
      {shouldMount && (
        ...
        >
          ...
          <Suspense fallback={null}>
            <SpeakerDriver motionAllowed={motionAllowed} />
          </Suspense>
        </Canvas>
      )}
    </div>
