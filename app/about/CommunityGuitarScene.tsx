...
const MODEL_PATH = "/3d/Meshy_AI_Vintage_Digital_Mixin_0829043853_optimized.glb";

function Guitar({ motionAllowed }: { motionAllowed: boolean }) {
  ...
  useFrame(({ clock, pointer }, delta) => {
    ...
    const targetX = motionAllowed ? 0.08 + pointer.y * 0.07 : 0.08;
    const targetY = motionAllowed
      ? -0.34 + pointer.x * 0.11 + Math.sin(clock.elapsedTime * 0.42) * 0.07
      : -0.34;
    ...
    object.position.y = motionAllowed
      ? Math.sin(clock.elapsedTime * 0.68 + 1.5) * 0.045
      : 0;
  });

  return (
    <group ref={group} rotation={[0.08, -0.34, -0.36]}>
      <primitive object={model.clone} scale={model.scale} />
    </group>
  );
}

export function CommunityGuitarScene() {
    ...
    <div ref={containerRef} style={{ width: "100%", height: "100%" }}>
      {shouldMount && (
        <Canvas
          ...
          camera={{ position: [0, 0.1, 6], fov: 32, near: 0.1, far: 100 }}
        ...
        >
          ...
          <Suspense fallback={null}>
            <Guitar motionAllowed={motionAllowed} />
          </Suspense>
        </Canvas>
      )}
    </div>
